...
func (m *Masker) URL(i string) string {
	...
	s3URLPattern := regexp.MustCompile(`s3\.(.*?)\.amazonaws\.com`)
	match := s3URLPattern.FindStringSubmatch(u.Host)
	if len(match) > 1 {
		u.Host = strings.ReplaceAll(u.Host, match[1], "**-**-**")
	}
	...
	for _, v := range strings.Split(path, "/") {
		...
		u.Path = strings.ReplaceAll(u.Path, v, m.String(MID, v))
	}
	queries := u.Query()
	for k, v := range queries {
		queries[k] = []string{m.String(MID, v[0])}
	}
	u.RawQuery = queries.Encode()

	finalUrl := u.Redacted()
	return strings.ReplaceAll(finalUrl, "%2A", "*")
}
